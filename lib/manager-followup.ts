...
export function followUpPrompt(params: { reporter: string; title: string; blocked: boolean; depth: number }): string {
  ...
  if (params.blocked) {
    ...
    lines.push(
      "- 이 결과가 아직 검토(QA) 팀원의 검토를 거치지 않았다면: read_task_result 로 결과 전문을 읽고, QA 팀원에게 delegate_task 로 검토를 맡기세요 (brief 에 결과 전문과 검토 기준을 그대로 넣습니다). QA 팀원이 없으면 recruit_agent 로 합류시킨 뒤 맡기세요. 검토를 생략하지 마세요.",
      "- 중간 결과는 사용자에게 안내하지 않습니다: 검토 전 보고에 대해 '결과 보기' 링크·파일 경로·요약을 사용자에게 내놓지 말고, 누구에게 무엇을 맡겼는지 한두 문장으로만 알리세요. 사용자에게는 검토까지 끝난 최종 결과만 안내합니다.",
      '- 방금 도착한 것이 검토 보고라면: important 지적이 결과물에 반영돼야 하면 작성 담당에게 수정을 한 번만 더 맡기고, 그렇지 않으면 최종 결과를 사용자에게 안내하며 끝내세요 — 무엇이 만들어졌고 어디에 저장됐는지(파일 경로), 핵심 내용 요약, 미확인·확인 필요 항목, 검토 결과 요약, 그리고 "프로젝트 상세의 \'결과보기\' 버튼으로 바로 열 수 있습니다" 안내. 파일 전문은 붙이지 마세요. 결과 링크는 산출물을 만든 작성 담당 카드로 겁니다 — 형식 [**\'제목\' 결과 보기**](#task/<카드 id>), 카드 id 는 \'이번 임무\' 섹션이나 보고 말풍선의 "카드 id" 에 있습니다 (검토 카드가 아니라 작성 카드).',
      '- 팀원이 산출물을 파일로 저장하지 않고 본문으로만 보고했다면(보고에 📎 산출물 파일이 없음), 검토를 맡기기 전에 같은 팀원에게 파일로 저장하도록 한 번만 다시 맡기세요.',
    );
  }
  lines.push(
