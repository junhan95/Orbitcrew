...
export function followUpPrompt(params: { reporter: string; title: string; blocked: boolean; depth: number }): string {
  ...
  if (params.blocked) {
    ...
    lines.push(
      ...
      '- 방금 도착한 것이 검토 보고라면: important 지적이 결과물에 반영돼야 하면 작성 담당에게 수정을 한 번만 더 맡기고, 그렇지 않으면 최종 결과를 사용자에게 안내하며 끝내세요 — 무엇이 만들어졌고 어디에 저장됐는지(파일 경로), 핵심 내용 요약, 미확인·확인 필요 항목, 검토 결과 요약, 그리고 "프로젝트 상세의 \'결과보기\' 버튼으로 바로 열 수 있습니다" 안내. 파일 전문은 붙이지 마세요. 결과 링크는 보고 메시지의 형식([**\'제목\' 결과 보기**](#task/<id>))을 그대로 쓰세요.',
